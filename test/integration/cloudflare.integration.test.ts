...
import { CloudflareD1Cache, CloudflareD1Leases, CloudflareKvCache } from '@/index';
...
describeMiniflareIntegration('cloudflare adapters integration', () => {
  ...
  it(
    'stores values and respects TTL with KV cache + D1 leases',
    async () => {
      ...
      await cache.set('k', 'value', Duration.seconds(60));
  ...
  it(
    'stores values and respects TTL with D1 cache + D1 leases',
    async () => {
