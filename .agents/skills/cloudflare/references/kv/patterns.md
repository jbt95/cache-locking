...
async function getFeatureFlags(env: Env): Promise<Record<string, boolean>> {
  return (await env.CONFIG.get<Record<string, boolean>>('features:flags', { type: 'json', cacheTtl: 600 })) || {};
}
...
async function getVariant(env: Env, userId: string, testName: string): Promise<string> {
  ...
  const test = await env.AB_TESTS.get<{ variants: string[]; weights: number[] }>(`test:${testName}:config`, {
    type: 'json',
    cacheTtl: 3600,
  });
