...
export class ChatAgent extends Agent<Env, ChatState> {
  ...
  async onMessage(conn: Connection, msg: WSMessage) {
    ...
    if (m.type === 'chat')
      this.setState({
        ...this.state,
        messages: [...this.state.messages, { userId: conn.state.userId, text: m.text, timestamp: Date.now() }],
      });
  }
}
```
...
export class AIAgent extends Agent<Env> {
  ...
  async onMessage(conn: Connection, msg: WSMessage) {
    ...
    conn.send(
      JSON.stringify({
        response: await this.env.AI.run('@cf/meta/llama-3.1-8b-instruct', { prompt: m.prompt, tools: this.tools }),
      }),
    );
  }
}
